...
async function leerLineas() { // defines una función asincrona. Esto para poder usar await dentro de ella y manejar los flujos de datos (Streams) de manera mas clara y bloqueante.
    const fileStream = fs.createReadStream('contenido.txt', 'utf8') // "fs.createReadStream" abre el archivo "contenido.txt" en modo lectura, pero no lo carga completo en memoria. Stream: Los datos se van cargando por partes (chunks), lo que es eficiente para archivos grandes
    const rl = readLine.createInterface({ // crea una interface de lectura por linea
        input: fileStream, // le decimos que la fuente de datos, sera el archivo que abrimos. "Como se hace referencia ene la linea 5"
